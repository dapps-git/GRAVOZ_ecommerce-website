...
export default function NewProductPage() {
  ...
  const [sku, setSku] = useState(() => 'GRV-' + Math.random().toString(36).substring(2, 8).toUpperCase());
  ...
  // 9. Shipping & Returns Policy (Pre-filled suggested defaults)
  ...
  const handleMockupUpload = async (field: 'mainUrl' | 'thumbnailUrl' | 'lifestyleUrl', file: File) => {
    ...
    try {
      const optimizedFile = await compressImage(file, { maxWidth: 1800, maxHeight: 1800, quality: 0.85 });
      const formData = new FormData();
      formData.append('file', optimizedFile);
      formData.append('folder', 'gravoz/mockups');
      const res = await fetch('/api/admin/upload', { method: 'POST', body: formData });
      const data = await res.json();
      if (res.ok && data.url) {
    ...
    try {
      ...
      const res = await fetch('/api/products', {
        ...
        body: JSON.stringify({
          ...
          status: targetStatus,
          spotlightMockups,
          featureInDuoSlot: featureInDuoSlot || undefined,
          shippingAndReturn: {
    ...
    <div className="w-full space-y-5 pb-24 font-sans font-normal" style={{ fontFamily: 'var(--font-montserrat), Montserrat, sans-serif' }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
        ...
        <main className="lg:col-span-6 xl:col-span-7 space-y-5">
            ...
            <div className="bg-white rounded-2xl p-5 border border-[#e8e2d8] space-y-5 shadow-2xs">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  ...
                  <label className={`p-3 rounded-lg border flex items-center gap-2.5 cursor-pointer transition-all ${noReturnRefundExchange ? 'bg-[#fff5f5] border-[#e53e3e] shadow-2xs' : 'bg-[#faf8f5] border-[#e8e2d8]'}`}>
                    <input
                      type="checkbox"
          ...
          {/* ── TAB 9: Homepage Duo Spotlight Mockups (3 Photos) ── */}
          {activeTab === 9 && (
            <div className="bg-white rounded-2xl p-5 sm:p-6 border border-[#e8e2d8] space-y-5 shadow-2xs">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#f0eae1] pb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-2 py-0.5 rounded text-[9px] font-bold bg-[#faf4ec] text-[#89591C] border border-[#ebdcc9] uppercase tracking-wider">
                    Optional
                  </span>
                </div>
