...
export default function ColorVariantManager({
  ...
  const [uploading, setUploading] = useState<Record<string, boolean>>({}); // key = `${variantIdx}-${imgIdx}`
    ...
    <div className="space-y-3 font-sansation">
      ...
      <div className="flex items-center justify-between">
        <label className="text-xs font-bold text-slate-900 uppercase tracking-wider block">
          Product Colors ({colorVariants.length} Added)
        </label>
        <span className="text-[11px] text-slate-400">Up to 6 photos per color</span>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
          ...
          return (
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer border ${
                isAdded
                  ? 'bg-slate-50 text-slate-400 opacity-60 cursor-not-allowed border-slate-200'
                  : 'bg-white border-[#e8e2d8] text-slate-800 hover:border-[#89591C]'
              }`}
            >
        ...
        {/* Custom color input */}
        <div className="flex items-center gap-1 bg-white border border-[#e8e2d8] rounded-xl px-2 py-0.5">
          <input
            type="color"
            value={customCode}
          ...
          <input
            ...
            className="w-28 text-xs text-slate-800 focus:outline-none placeholder-slate-400"
          />
          <button
            type="button"
            onClick={() => addColor(customName, customCode)}
            className="text-xs font-bold text-[#89591C] hover:underline px-1"
          >
            + Add
          </button>
        </div>
      ...
      {colorVariants.length > 0 && (
        <div className="space-y-3 pt-1">
          {colorVariants.map((item, variantIdx) => {
            ...
            return (
              <div key={item.id || variantIdx} className="p-4 bg-white rounded-2xl border border-[#e8e2d8] space-y-3 shadow-2xs">
                {/* Color Header */}
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="w-4 h-4 rounded-full border border-black/20" style={{ backgroundColor: item.colorCode || '#000' }} />
                    <span className="text-xs font-bold text-slate-900">{item.name}</span>
                    <span className="text-[11px] text-slate-400">{varImages.length}/{MAX_COLOR_IMAGES} photos</span>
                  </div>
                  <button
                    type="button"
                ...
                <div className="flex flex-wrap gap-2">
                  {varImages.map((img, imgIdx) => (
                    <div key={imgIdx} className="relative w-16 h-16 rounded-xl overflow-hidden border border-[#e8e2d8] bg-[#faf8f5] group flex-shrink-0">
                      ...
                      {imgIdx === 0 && (
                        <span className="absolute bottom-0.5 left-0.5 text-[8px] bg-[#89591C] text-white px-1 rounded font-bold">Main</span>
                      )}
                      <button
                        type="button"
                        onClick={() => handleRemoveImage(variantIdx, imgIdx)}
                        className="absolute top-0.5 right-0.5 w-5 h-5 rounded-full bg-rose-600 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
                      >
                        <X className="w-3 h-3" />
                      </button>
                    </div>
