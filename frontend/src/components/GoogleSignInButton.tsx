'use client';

import React, { useEffect, useState } from 'react';
import { loginWithGoogle, AuthUser } from '@/lib/auth-client';
import { playSuccessSound } from '@/lib/sounds';

interface Props {
  onSuccess: (user: AuthUser) => void;
  onError: (error: string) => void;
  isSubmitting?: boolean;
  setIsSubmitting?: (val: boolean) => void;
  text?: string;
}

declare global {
  interface Window {
    google?: any;
  }
}

export default function GoogleSignInButton({
  onSuccess,
  onError,
  isSubmitting,
  setIsSubmitting,
  text = 'Continue with Google',
}: Props) {
  const [isGsiLoaded, setIsGsiLoaded] = useState(false);
  const clientId = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID || '';

  // Load Google Identity Services SDK
  useEffect(() => {
    if (typeof window === 'undefined') return;

    if (window.google?.accounts?.id) {
      setIsGsiLoaded(true);
      return;
    }

    const script = document.createElement('script');
    script.src = 'https://accounts.google.com/gsi/client';
    script.async = true;
    script.defer = true;
    script.onload = () => {
      setIsGsiLoaded(true);
    };
    document.body.appendChild(script);

    return () => {
      if (document.body.contains(script)) {
        document.body.removeChild(script);
      }
    };
  }, []);

  const handleCredentialResponse = async (response: any) => {
    if (setIsSubmitting) setIsSubmitting(true);
    try {
      if (!response.credential) {
        throw new Error('No Google credential returned');
      }

      const res = await loginWithGoogle({ credential: response.credential });
      if (res.success && res.user) {
        playSuccessSound();
        onSuccess(res.user);
      } else {
        onError(res.error || 'Google sign-in failed');
      }
    } catch (err: any) {
      onError(err.message || 'Google authentication error');
    } finally {
      if (setIsSubmitting) setIsSubmitting(false);
    }
  };

  const handleGoogleClick = () => {
    if (!clientId) {
      // If Google Client ID is not yet entered in .env.local, prompt the user with clear instructions
      const demoEmail = prompt(
        'Google OAuth Client ID is not configured yet.\n\nTo test Google login right now, enter any email (e.g. yourname@gmail.com):',
        'customer@gmail.com'
      );
      if (demoEmail) {
        if (setIsSubmitting) setIsSubmitting(true);
        loginWithGoogle({
          email: demoEmail.trim(),
          name: demoEmail.split('@')[0],
          googleId: 'demo-google-' + Date.now(),
        }).then((res) => {
          if (res.success && res.user) {
            playSuccessSound();
            onSuccess(res.user);
          } else {
            onError(res.error || 'Google login failed');
          }
        }).catch((err) => {
          onError(err.message || 'Google authentication error');
        }).finally(() => {
          if (setIsSubmitting) setIsSubmitting(false);
        });
      }
      return;
    }

    if (window.google?.accounts?.id) {
      try {
        window.google.accounts.id.initialize({
          client_id: clientId,
          callback: handleCredentialResponse,
          auto_select: false,
          cancel_on_tap_outside: true,
        });

        // Trigger Google One-Tap prompt or Account Chooser
        window.google.accounts.id.prompt((notification: any) => {
          if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
            // Render popup button fallback
            const btnContainer = document.getElementById('google-hidden-btn');
            if (btnContainer) {
              window.google.accounts.id.renderButton(btnContainer, {
                theme: 'outline',
                size: 'large',
                width: '100%',
              });
              const actualBtn = btnContainer.querySelector('div[role=button]') as HTMLElement;
              if (actualBtn) actualBtn.click();
            }
          }
        });
      } catch (err: any) {
        onError(err.message || 'Failed to initialize Google Sign-in');
      }
    } else {
      onError('Google services are still loading. Please try in a moment.');
    }
  };

  return (
    <>
      <div id="google-hidden-btn" className="hidden" />
      <button
        type="button"
        onClick={handleGoogleClick}
        disabled={isSubmitting}
        className="w-full flex items-center justify-center gap-2.5 py-2.5 border border-[#ddd6cc] bg-white hover:bg-[#faf9f7] text-slate-700 text-[11.5px] font-medium transition-colors cursor-pointer font-sansation disabled:opacity-60"
      >
        <svg className="w-4 h-4 flex-shrink-0" viewBox="0 0 24 24">
          <path
            fill="#4285F4"
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
          />
          <path
            fill="#34A853"
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
          />
          <path
            fill="#FBBC05"
            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"
          />
          <path
            fill="#EA4335"
            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"
          />
        </svg>
        {isSubmitting ? 'Connecting...' : text}
      </button>
    </>
  );
}
